...
export const FetchUpdateUserInfo = async (request: { name: string; phoneNum: string }) => {
...
export const FetchUpdatePassword = async (prevPW: string, newPW: string, newPWConfirm: string) => {
  const session = await getSession()
  ...
  return Api.put("/v1/api/user/update/password", request, {
    ...
    .then((res) => {
      if (res.status === 200) {
        confirmAlert("success", "비밀번호 변경 완료", "다시 로그인해주세요.").then((res) => {
          if (res.isConfirmed) {
            signOut()
    ...
    .catch((err) => {
      ...
      if (message === "비어 있을 수 없습니다") confirmAlert("warning", "빈칸을 모두 채워주세요.")
        ...
        confirmAlert("warning", "비밀번호 오류", "비밀번호와 비밀번호 확인이 같지 않습니다.")
