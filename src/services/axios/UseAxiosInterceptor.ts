...
const useAxiosInterceptor = (): void => {
  const { AccessToken, setAccessToken, isHydrated } = useUserStore();
  useEffect(() => {
    // Hydration이 완료되지 않은 경우 인터셉터를 설정하지 않음
    if (!isHydrated) {
      return;
    }
    const requestHandler = async (config: InternalAxiosRequestConfig) => {
      ...
      if (AccessToken) {
        ...
        const expireTime: number = Math.floor(
          new Date(JwtDecoder(AccessToken).exp).getTime(),
        );
  ...
  }, [AccessToken, isHydrated, setAccessToken]);
};
